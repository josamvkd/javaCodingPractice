...
public class Java8StreamMainClass {
    public static void main(String[] args) {
        ...
        //2. Find the Students who stays in Karnataka and sort them by their names
            // 1.) Ascending order
                List<Student> studentsByCityAsc = studentList
                        .stream()
                        .filter(student -> student.getCity().equals("Karnataka"))
                        .sorted(Comparator.comparing(Student::getFirstName))
                        .collect(Collectors.toList());
                //System.out.print(studentsByCityAsc);
           // 2.) Descending order
                List<Student> studentsByCityDesc = studentList
                        .stream()
                        .filter(student -> student.getCity().equals("Karnataka"))
                        .sorted(Comparator.comparing(Student::getFirstName, Comparator.reverseOrder()))
                        .collect(Collectors.toList());
                System.out.print(studentsByCityDesc);
        //3. Find all departments names
            List<String> departmentList1= studentList.stream().map(Student::getDept).distinct().collect(Collectors.toList());
            // or
            Set<String> departmentList2= studentList.stream().map(Student::getDept).collect(Collectors.toSet());
            //System.out.println(departmentList1);
            //System.out.println(departmentList2);
        //4. Find all the contact numbers
            List<List<String>> contactsList1= studentList.stream().map(Student::getContacts).distinct().collect(Collectors.toList());
            //System.out.println(contactsList1);
            //one2one-> map
            List<String> contactsList2= studentList.stream().flatMap(student -> student.getContacts().stream()).collect(Collectors.toList());
            //System.out.println(contactsList2);
            //one2many-> flatmap
        //5. Group The Student By Department Names
        Map<String,List<Student>> studentByDepartment = studentList.stream().collect(Collectors.groupingBy(Student::getDept));
